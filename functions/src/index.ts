...
const sendPaymentNotification = async (churchData: any, paymentDate: string, amount: number) => {
  try {
    const emailBody = `
    ...
    await sendEmailBatch(
      ['finance@g3min.org'],
      'G3 Church Network Dues Payment Received',
      emailBody,
      SYSTEM_SENDER
    );
  ...
  try {
    ...
    if (error.code === 'resource_missing') {
      ...
      try {
        ...
        throw new HttpsError("not-found", "Customer not found in either live or test mode.");
      }
    }
 ...
 */
export const provisionallyApproveApplication = onCall({ cors: true }, async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "Must be authenticated.");
  }
  ...
  try {
    ...
    const body = replaceVariables(template.body, { ...appData, resetLink });
 ...
 */
export const processChurchPayment = onCall({ cors: true }, async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "Must be authenticated.");
  }
  ...
  try {
    ...
    if (appData.userId !== request.auth.uid) {
      throw new HttpsError("permission-denied", "You don't have permission to process payment for this application.");
    }
    ...
    if (!allowedStatuses.includes(appData.status)) {
      throw new HttpsError("failed-precondition", `Application status is ${appData.status}, expected one of: ${allowedStatuses.join(', ')}.`);
    }
    ...
    if (paymentFrequency === 'yearly') {
      ...
      if (subscription.status === 'active') {
        ...
        throw new HttpsError("aborted", `Subscription creation failed with status: ${subscription.status}`);
      }
    ...
    await docRef.update({
      status: 'APPROVED',
      isManuallyDelinquent: false, // Clear manual delinquency if present
      paymentAmount,
      paymentFrequency,
...
export const checkDuesAndReminders = onSchedule({
 ...
 */
export const syncSubscriptionStatus = onCall({ cors: true }, async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "Must be authenticated.");
  }
  ...
  try {
    ...
    const appData = docSnap.data() as ChurchApplication; // Cast to ChurchApplication
    if (!appData) {
      throw new HttpsError("not-found", "No data for application.");
    }
    ...
    if (subscription.cancel_at_period_end) {
      ...
      if (appData.status !== 'DELINQUENT') {
        await docRef.update({
          status: 'DELINQUENT',
          paymentFrequency: 'one_time', // Assume manual payment if subscription fully canceled
          updatedAt: toSafeISOString(new Date())
        });
    ...
    if (error.code === 'resource_missing' && error.raw?.param === 'subscription') {
      // Handle cases where the subscription might have been deleted in Stripe
      try {
        const docRef = db.collection('applications').doc(churchId);
        const appData = (await docRef.get()).data() as ChurchApplication; // Cast here too
        if (appData?.paymentFrequency === 'yearly') {
          await docRef.update({
            paymentFrequency: 'one_time', // Revert to one-time if subscription gone
            'stripeSubscriptionId': admin.firestore.FieldValue.delete(), // Remove the stale ID
            updatedAt: toSafeISOString(new Date())
          });
        ...
        throw new HttpsError("internal", `Failed to update Firestore after missing Stripe subscription: ${dbError}`);
      }
    }
    throw new HttpsError("internal", error.message);
  }
...
export const onApplicationCreatedShopifySync = onDocumentCreated({
  document: "applications/{id}",
  database: DATABASE_ID
}, async (event) => {
  const snap = event.data;
  ...
  const application = snap.data() as ChurchApplication;
  if (!application) return;
...
export const onApplicationUpdatedShopifySync = onDocumentUpdated({
  document: "applications/{id}",
  database: DATABASE_ID
}, async (event) => {
  const before = event.data?.before.data() as ChurchApplication;
  const after = event.data?.after.data() as ChurchApplication;
  ...
  try {
    ...
    let customers = (searchResponse.body as any).customers;
    ...
    if (!customers || customers.length === 0) {
      ...
      customers = (searchResponse.body as any).customers;
    }
    ...
    if (customers && customers.length > 0) {
      ...
      const responseBody = createResponse.body as any;
      if (responseBody.customer && responseBody.customer.id) {
...
export const changeUserPassword = onCall({ cors: true }, async (request) => {
  if (!isAdmin(request)) {
  ...
  try {
    ...
    await getAuth(admin.app()).updateUser(uid, {
...
export const sendPasswordResetEmail = onCall({ cors: true }, async (request) => {
  if (!isAdmin(request)) {
    throw new HttpsError("permission-denied", "Only administrators can send password reset emails.");
  }
  ...
  try {
    ...
    const link = await getAuth(admin.app()).generatePasswordResetLink(email, {
      url: CHURCH_LOGIN_URL
    });
...
export const regeocodeAddress = onCall({ cors: true }, async (request) => {
  ...
  try {
    ...
    if (!addr.city || !addr.country) {
      ...
      throw new HttpsError("failed-precondition", "Incomplete address: city and country are required for geocoding.");
    }
 ...
 */
export const clearTestStripeData = onCall({ cors: true }, async (request) => {
  if (!isAdmin(request)) {
    throw new HttpsError("permission-denied", "Only administrators can clear test Stripe data.");
  }
...
export const createStripeCheckoutSession = onCall({ cors: true }, async (request) => {
  ...
  if (paymentAmount < 500) {
    throw new HttpsError("invalid-argument", "Minimum annual payment amount is $500.");
  }

  // Determine payment plan and installment amount
  const plan: 'annual' | 'biannual' | 'quarterly' = paymentPlan || 'annual';
  ...
  try {
    ...
    if (!churchData) {
      throw new HttpsError("not-found", "No church data found.");
    }

    // Verify the authenticated user owns this church
    if (request.auth.uid !== churchData.userId) {
      throw new HttpsError("permission-denied", "You do not have permission to manage this church's payment.");
    }
    ...
    const recurringConfig: Stripe.PriceCreateParams.Recurring = plan === 'quarterly'
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (subscriptionId) {
          ...
          const nextDueDate = toSafeISOString(new Date((subscription as any).current_period_end * 1000));

          // Extract payment plan metadata
          const sessionPlan = session.metadata?.paymentPlan || 'annual';
          const sessionInstallmentAmount = session.metadata?.installmentAmount ? parseFloat(session.metadata.installmentAmount) : (paymentAmount ? parseFloat(paymentAmount) : 500);
          ...
          const updateData: any = {
            ...
            totalPaidInPeriod: sessionInstallmentAmount, // First installment paid
      ...
      case 'invoice.payment_succeeded': {
        ...
        if (subscriptionId) {
          ...
          if (!snapshot.empty) {
            ...
            const amountPaid = invoice.amount_paid ? invoice.amount_paid / 100 : (churchData?.installmentAmount || churchData?.paymentAmount || 0);

            // Determine payment plan from church data
            const currentPlan = churchData?.paymentPlan || 'annual';
            const currentInstallmentCount = currentPlan === 'quarterly' ? 4 : currentPlan === 'biannual' ? 2 : 1;
            const previousPaidCount = churchData?.installmentsPaidCount || 0;
            const previousTotalPaid = churchData?.totalPaidInPeriod || 0;
            const newPaidCount = previousPaidCount + 1;
            const newTotalPaid = previousTotalPaid + amountPaid;

            // Calculate next installment due date based on plan
            let nextInstallmentDue: string | null = null;
            ...
            if (cycleComplete) {
              ...
              const annualStart = churchData?.annualPeriodStart ? new Date(churchData.annualPeriodStart) : new Date(lastPaymentDate!);
              ...
              nextDueDate = churchData?.nextDueDate || null;
              if (!nextDueDate) {
            ...
            if (cycleComplete && newTotalPaid >= (churchData?.paymentAmount || 500)) {
              console.log(`[Installment] Annual cycle complete for ${churchData?.churchName}. Total paid: $${newTotalPaid}. Resetting for next year.`);
            ...
            console.log(`[Installment] Updated church ${churchDoc.id} - Payment #${newPaidCount}/${currentInstallmentCount}, Total: $${newTotalPaid}/$${churchData?.paymentAmount || 500}`);
      ...
      case 'payment_intent.succeeded': {
        ...
        const churchId = paymentIntent.metadata?.churchId;
        ...
        if (churchId && (paymentType === 'annual_dues_onetime' || paymentType === 'annual_dues')) {
          ...
          if (churchDoc.exists) {
            const churchData = churchDoc.data();
            const lastPaymentDate = toSafeISOString(new Date((paymentIntent as any).created * 1000));
